...
function SidebarNav({ user, onCompose, collapsed, toggleCollapsed }) {
  ...
  const getNavLabel = (label) => (label === 'Notifications' ? 'Alerts' : label);
  ...
  return (
    ...
    >
      <div className="panel nav-panel">
        <button className="ghost-button sidebar-toggle" type="button" onClick={toggleCollapsed} aria-label={isCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}>
          {isCollapsed ? '☰' : '◀'}
        </button>
        ...
        {links.map((link) => (
          <Link key={link.path} className={`nav-item ${isActive(link.path) ? 'active' : ''}`} to={link.path} title={link.label} aria-label={getNavLabel(link.label)}>
        ...
        {onCompose ? (
          <button className="primary-button" type="button" onClick={onCompose} aria-label="Compose" title="Compose">
            {isCollapsed ? '✚' : 'Compose'}
          </button>
        ) : (
          <Link className="primary-button sidebar-link" to="/" aria-label="Back home" title="Back home">
